...
namespace DrawTools
{
	...
	/// Polygon tool
	/// </summary>
	public class ToolLink : DrawTools.ToolObject
	{
		public ToolLink(DrawArea da)
		{
            Owner = da;
        ...
        public override void OnMouseDown(DrawArea drawArea, MouseEventArgs e)
        {
            ...
            if (drawArea.AddObjet)
            {
                ...
                dl = (DrawLink)Owner.GraphicsList[0];
                dl.Add3Points(point);
                //dl..rectangle.X = e.X; du.rectangle.Y = e.Y;
                dl.Normalize();
                dl.Selected = true;
            } else AddNewObject(drawArea, new DrawLink(drawArea.Owner, point.X, point.Y, point.X, point.Y, drawArea.GraphicsList.Count), true);
            if (oAttach != null)
            {
                dl = (DrawLink) drawArea.GraphicsList[0];
        ...
        {
            DrawLink dl = (DrawLink)drawArea.GraphicsList[0];
            Point p0 = (Point)dl.pointArray[0];
            Point p1 = (Point)dl.pointArray[1];
        ...
        public override void OnMouseMove(DrawArea drawArea, MouseEventArgs e)
        {
            ...
            {
                Point point = new Point(e.X, e.Y);
                o = (DrawLink)drawArea.GraphicsList.GetSelectedObject(0);
        ...
        {
            return "From DansVue, " + sType + " Left Join LayerLink On " + sType + ".Guid" + sType + "=GuidObj" + " and layerlink.GuidAppVersion='" + Owner.Owner.GetGuidAppVersion() + "', " + sGType + ", GPoint";
        ...
        {
            return "WHERE GuidGVue ='" + GuidGVue + "' and GuidObjet=Guid" + sGType + " and " + sGType + ".Guid" + sType + "=" + sType + ".Guid" + sType + " AND " + sGType + ".Guid" + sGType + "=GPoint.GuidGObjet" + Owner.Owner.wkApp.GetWhereLayer() + " ORDER BY Id DESC, Guid" + sGType + ", I";
        ...
        public override DrawObject CreatObjetFromJson(Dictionary<string, object> dic)
        {
            ...
            dl = new DrawLink(Owner.Owner, dic);
            if (Owner.Owner.oCureo != null) Owner.Owner.oCureo.oDraw = dl;
            else attachEtCreatLink(dl, false);
            ...
            for (int j = 0; j < aPoints.Count; j++)
            {
                ...
                dl.pointArray.Add(new Point(Convert.ToInt32(jo.GetValue("x").ToString()), Convert.ToInt32(jo.GetValue("y").ToString())));
            }

            return dl;
        }

        public void attachEtCreatLink(DrawLink dl, bool selected)
        {
            ...
            switch (sTypeVue[0])
            {
                case '0': //0-Fonctionnelle
                    sIn = "GuidModuleIn"; sOut = "GuidModuleOut";
                    break;
                case 'U':
                case '1': //1-Applicative
                    sIn = "GuidComposantL1In"; sOut = "GuidComposantL1Out";
                    break;
                case 'V': //V-SI App
        ...
        public override void CreatObjetFromBD(ArrayList LstValue, ArrayList LstValueG, bool bSelectedRoot = false)
        {
            ...
            if (n > -1)
            {
                dl = (DrawLink)Owner.GraphicsList[n];
                dl.AddPoint(LstValueG);
            }
            else
            {
                //selected = true;
                dl = new DrawLink(Owner.Owner, LstValue, LstValueG);
                if (dl.pointArray.Count == 0) { dl.Add3Points(new Point(0, 0)); }
